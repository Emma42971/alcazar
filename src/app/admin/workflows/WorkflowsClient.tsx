...
type Rule = { id: string; name: string; trigger: string; action: string; isActive: boolean; executions: { id: string; status: string; executedAt: string }[] }
      ...
      {rules.length === 0 ? (
        ...
        <div className="space-y-3">
          {rules.map(rule => {
            ...
            const successCount = rule.executions.filter(e => e.status === "success").length
            return (
                  ...
                  <div className="flex-1">
                    ...
                    {rule.executions.length > 0 && (
                      <p className="text-xs mt-2" style={{ color: "hsl(var(--text-muted))" }}>
                        Last run: {new Date(rule.executions[0].executedAt).toLocaleString()} · {successCount} successful
                      </p>
                    )}
                  </div>
