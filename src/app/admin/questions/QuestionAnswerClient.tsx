...
export function QuestionAnswerClient({ questionId }: { questionId: string }) {
  ...
  return (
    <div className="mt-3 space-y-2">
      <textarea value={answer} onChange={e => setAnswer(e.target.value)} rows={3} placeholder="Your answer (visible only to this investor)…" className="w-full rounded-lg px-3 py-2 text-sm outline-none resize-none" style={{ background: "hsl(var(--surface))", border: "1px solid hsl(var(--border))", color: "hsl(var(--text))" }}/>
