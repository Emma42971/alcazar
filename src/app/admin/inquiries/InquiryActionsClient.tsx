...
export function InquiryActionsClient({ inquiryId, currentStatus, adminNote }: { inquiryId: string; currentStatus: string; adminNote: string | null }) {
  ...
  if (currentStatus === "REPLIED") return adminNote ? <p className="text-xs italic" style={{ color: "hsl(var(--text-subtle))" }}>Note: {adminNote}</p> : null
  return (
    <div className="space-y-2 pt-2 border-t" style={{ borderColor: "hsl(var(--border))" }}>
      <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder="Internal note (optional)…" className="w-full rounded-lg px-3 py-2 text-xs outline-none resize-none" style={{ background: "hsl(var(--surface))", border: "1px solid hsl(var(--border))", color: "hsl(var(--text-subtle))" }}/>
      <div className="flex gap-2">
        {currentStatus === "NEW" && <button onClick={() => update("READ")} disabled={loading} className="text-xs px-2.5 py-1 rounded-lg border" style={{ borderColor: "hsl(var(--border))", color: "hsl(var(--text-subtle))" }}>Mark Read</button>}
